    ...
    <section id="process" className="bg-[#fdfcf8] py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, idx) => (
            <div key={idx} className="group relative bg-white p-8 md:p-10 lg:p-12 rounded-[40px] border border-accent/5 shadow-premium hover:shadow-premium-lg hover:translate-y-[-10px] transition-all duration-1000 flex flex-col items-center text-center">
              <div className="w-20 h-20 rounded-3xl bg-sand/20 flex items-center justify-center text-accent mb-10 group-hover:bg-accent group-hover:text-white transition-all duration-700 shadow-inner mx-auto">
                {React.cloneElement(step.icon, { size: 28, strokeWidth: 1.2 })}
              </div>
              
              <h3 className="text-2xl mb-6 font-serif italic text-accent group-hover:text-gold transition-colors duration-500 w-full">
                {step.title}
              </h3>
              
              <p className="text-[13px] md:text-[14px] leading-loose text-accent/60 font-light tracking-wide group-hover:text-accent transition-colors duration-500 w-full px-2">
                {step.desc}
              </p>

              {/* Decorative Number */}
              <span className="absolute top-6 right-8 text-4xl font-serif italic opacity-5 text-accent group-hover:opacity-10 transition-opacity">
                0{idx + 1}
              </span>
